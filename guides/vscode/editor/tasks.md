...
If you are using Visual Studio Code version 1.13 or earlier, refer to the previous version of the Tasks [documentation](tasks-v1.md).
...
You can also review the [tasks.json schema](tasks-appendix.md).
...
You can also create your own problem matcher, which we'll discuss [in a later section](tasks.md#defining-a-problem-matcher).
...
When authoring tasks configurations, it is useful to have a set of predefined common variables such as the active file (`${file}`) or workspace root folder (`${workspaceFolder}`). VS Code supports variable substitution inside strings in the `tasks.json` file and you can see a full list of predefined variables in the [Variables Reference](variables-reference.md).
...
For more information about `inputs`, see the [Variables Reference](variables-reference.md).
...
The [TypeScript topic](../languages/typescript.md#transpiling-typescript-into-javascript) includes an example that creates a task to transpile TypeScript to JavaScript and observe any related errors from within VS Code.

### Compiling Markdown to HTML

The Markdown topic provides two examples for compiling Markdown to HTML:

1. [Manually compiling with a Build task](../languages/markdown.md#compiling-markdown-into-html)
2. [Automating the compile step with a file watcher](../languages/markdown.md#automating-markdown-compilation)

### Transpiling Less and SCSS into CSS

The CSS topic provides examples of how to use Tasks to generate CSS files.

1. [Manually transpiling with a Build task](../languages/css.md#transpiling-sass-and-less-into-css)
2. [Automation of the compile step with a file watcher](../languages/css.md#automating-sassless-compilation)
...
**Note**: If you have created a workspace that consists of multiple folders ([Multi-root Workspace](multi-root-workspaces.md)), only version `2.0.0` tasks are detected and shown in the **Terminal** > **Run Task** picker.
...
* [tasks.json Schema](tasks-appendix.md) - You can review the full `tasks.json` schema and descriptions.
* [Basic Editing](codebasics.md) - Learn about the powerful VS Code editor.
* [Code Navigation](editingevolved.md) - Move quickly through your source code.
* [Language Support](../languages/overview.md) - Learn about our supported programming languages, both shipped with VS Code and through community extensions.
* [Debugging](debugging.md) - Debug your source code directly in the VS Code editor.
...
For more on background tasks, go to [Background / watching tasks](tasks.md#background-watching-tasks).
