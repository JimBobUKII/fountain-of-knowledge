...
Visual Studio Code includes a fully featured integrated terminal that conveniently starts at the root of your workspace. It provides integration with the editor to support features like [links](#links) and [error detection](tasks.md).
...
>**Tip:** Path, args, and env all support [resolving variables](https://code.visualstudio.comvariables-reference)
...
Customize the terminal's appearance using the following [settings](https://code.visualstudio.com../getstarted/settings):
...
This feature supports [variable substitution](variables-reference.md).
...
The [Tasks](tasks.md) feature can be used to automate the launching of terminals, for example the following `.vscode/tasks.json` file will launch a Command Prompt and PowerShell terminal in a single terminal group when the window starts:
...
* [Tasks](tasks.md) - Tasks let you integrate with external tools and leverage the terminal heavily.
...
Yes. Specify [Powerline](https://powerline.readthedocs.io) fonts with the `terminal.integrated.fontFamily` [setting](../getstarted/settings.md).
...
Unfortunately some issues cannot be automatically detected, if you experience issues with the GPU acceleration you can disable it `terminal.integrated.gpuAcceleration` in your user or workspace [settings](../getstarted/settings.md), which will use the DOM renderer. This can be driven by the follow setting:
