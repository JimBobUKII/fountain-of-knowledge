...
The association of a JSON file to a schema can be done either in the JSON file itself using the `$schema` attribute, or in the User or Workspace [settings](../getstarted/settings.md) (**File** > **Preferences** > **Settings**) under the property `json.schemas`.
...
The following excerpt from User [Settings](../getstarted/settings.md) shows how `.babelrc` files are mapped to the [babelrc](https://babeljs.io../usage/babelrc) schema located on [https://json.schemastore.org/babelrc](https://json.schemastore.org/babelrc).
...
If you want your descriptions to support formatting like links, you can opt in by using [Markdown](../markdown.md) in your formatting with the `markdownDescription` property.
