...
1. Start debugging (use the `kb(workbench.action.debug.start)` key).
    ...
    >
    > You can configure the application to wait for the debugger to attach before starting execution by setting the [inspectMode](reference.md#node-object-properties-dockerrun-task) property to `break` in the `docker-run: debug` task in `tasks.json` under the `node` object.
...
- [Learn about debugging Node.js in a container](debug-node.md)
