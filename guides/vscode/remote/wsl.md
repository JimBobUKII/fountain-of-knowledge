...
**Note**: After reviewing this topic, you can get started with the introductory [WSL tutorial](wsl-tutorial.md).
...
If you are working with the same repository in WSL and Windows, be sure to set up consistent line endings. See [tips and tricks](troubleshooting.md#resolving-git-line-ending-issues-in-containers-resulting-in-many-modified-files) for details.

You can also avoid passwords by configuring WSL to use the Windows Git credential manager. See [tips and tricks](troubleshooting.md#sharing-git-credentials-between-windows-and-wsl) for details.
...
If you are using WSL 2 and [Docker Desktop's WSL 2 back-end](https://docs.docker.com/docker-for-windows/wsl-tech-preview/), you can use the [Remote - Containers](containers.md) extension to work with source code stored inside WSL! Just follow these steps:
...
See the [Remote - Containers documentation](containers.md) for more information.
...
In addition, some extensions installed in an WSL when using an Alpine Linux-based distribution may not work due to `glibc` dependencies in native code inside the extension. See the [Remote Development with Linux](linux.md) article for details.
...
All other communication between the server and the VS Code client is accomplished through an random local TCP port. You can find a list of locations VS Code itself needs access to in the [network connections article](../setup/network.md#common-hostnames).
...
- as described in [Network Connections in Visual Studio Code](../setup/network)
...
- manually in the [server setup script](wsl.md#advanced-environment-setup-script)
...
- See [Tips and Tricks](troubleshooting.md#wsl-tips) or the [FAQ](faq.md).
