...
* Add more [advanced container configurations](containers-advanced.md).
...
* [Use Docker or Kubernetes](containers-advanced.md#using-docker-or-kubernetes-from-a-container) from inside a dev container to build and deploy your app.

If `devcontainer.json`'s supported workflows do not meet your needs, you can also [attach to an already running container instead](attach-container.md).
...
4. Use separate VS Code windows to [work with multiple Docker Compose-defined services](containers-advanced.md#connecting-to-multiple-containers-at-once) at once.
...
See the [devcontainer.json reference](devcontainerjson-reference.md) for information other available properties such as the `workspaceFolder` and `shutdownAction`.
...
See [install additional software](#install-additional-software) for more information on installing software and the [devcontainer.json reference](devcontainerjson-reference.md) for more information about the `postCreateCommand` property.
...
First, update the **Remote > Containers: Repository Configuration Paths** [User setting](../getstarted/settings.md) with the local folder you want to use to store your repository container configuration files.
