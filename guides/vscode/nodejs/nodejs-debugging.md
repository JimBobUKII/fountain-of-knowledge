...
MetaSocialImage: /assets../editor/debugging/Debugging.png
---
...
In this section we'll go into more detail about configurations and features for more advanced debugging scenarios. You'll find instruction for debugging with [source maps](nodejs-debugging.md#source-maps), [stepping over external code](nodejs-debugging.md#skipping-uninteresting-code), doing [remote debugging](nodejs-debugging.md#remote-debugging), and much more.
...
>**Note**: If you are just getting started with VS Code, you can learn about general debugging features and creating `launch.json` configuration files in the [Debugging](../editor/debugging.md) topic.

## Launch configuration attributes

Debugging configurations are stored in a `launch.json` file located in your workspace's `.vscode` folder. An introduction into the creation and use of debugging configuration files is in the general [Debugging](../editor/debugging.md#launch-configurations) article.
...
* `outFiles` - array of glob patterns for locating generated JavaScript files. See section [Source maps](nodejs-debugging.md#source-maps).
* `resolveSourceMapLocations` - an array of glob patterns for locations where source maps should be parsed. See section [Source maps](nodejs-debugging.md#source-maps).
* `timeout` - when restarting a session, give up after this number of milliseconds. See section [Attaching to Node.js](nodejs-debugging.md#attaching-to-nodejs).
* `stopOnEntry` - break immediately when the program launches.
* `localRoot` - VS Code's root directory. See section [Remote debugging](nodejs-debugging.md#remote-debugging) below.
* `remoteRoot` - Node's root directory. See section [Remote debugging](nodejs-debugging.md#remote-debugging) below.
* `smartStep`- try to automatically step over code that doesn't map to source files. See section [Smart stepping](nodejs-debugging.md#smart-stepping).
* `skipFiles` - automatically skip files covered by these glob patterns. See section [Skipping uninteresting code](nodejs-debugging.md#skipping-uninteresting-code).
...
* `runtimeExecutable` - absolute path to the runtime executable to be used. Default is `node`. See section [Launch configuration support for 'npm' and other tools](nodejs-debugging.md#launch-configuration-support-for-npm-and-other-tools).
* `runtimeArgs` - optional arguments passed to the runtime executable.
* `runtimeVersion` - if "[nvm](https://github.com/creationix/nvm)" (or "[nvm-windows](https://github.com/coreybutler/nvm-windows)") or "[nvs](https://github.com/jasongin/nvs)" is used for managing Node.js versions, this attribute can be used to select a specific version of Node.js. See section [Multi version support](nodejs-debugging.md#multi-version-support) below.
...
* `envFile` - optional path to a file containing environment variable definitions. See section [Load environment variables from external file](nodejs-debugging.md#load-environment-variables-from-external-file-node) below.
* `console` - the console to launch the program (`internalConsole`, `integratedTerminal`, `externalTerminal`). See section [Node Console](nodejs-debugging.md#node-console) below.
...
* `restart` - restart the connection on termination. See section [Restarting debug session automatically](nodejs-debugging.md#restarting-debug-sessions-automatically-when-source-is-edited).
* `protocol` - debug protocol to use. See section [Supported Node-like runtimes](nodejs-debugging.md#supported-nodelike-runtimes) above.
* `port` - debug port to use. See sections [Attaching to Node.js](nodejs-debugging.md#attaching-to-nodejs) and [Remote debugging](nodejs-debugging.md#remote-debugging).
* `address` - TCP/IP address of the debug port. See sections [Attaching to Node.js](nodejs-debugging.md#attaching-to-nodejs) and [Remote debugging](nodejs-debugging.md#remote-debugging).
...
For Babel, you'll want to set the [sourceMaps](https://babeljs.io../en/options#sourcemaps) option to `true`, or pass the `--source-maps` option when compiling your code.
...
> **Note:** VS Code now has universal [remote development capabilities](../remote/remote-overview.md). Using the [Remote Development](https://aka.ms/vscode-remote/download/extension) extensions, Node.js development in remote scenarios and containers is no different than Node.js development in a local setup. **This is the recommended way to remote debug Node.js programs**. Check out the [Getting started](../remote/remote-overview.md#getting-started) section and [Remote tutorials](../remote/remote-overview.md#remote-tutorials) to learn more.
...
* [Node.js](nodejs-tutorial.md) - End to end Node scenario with a sample application

To see tutorials on the basics of Node.js debugging, check out these videos:

* [Intro Video - Debugging](../introvideos/debugging.md) - Introductory video showcasing the basics of debugging.
...
* [Tasks](../editor/tasks.md) - Running tasks with Gulp, Grunt, and Jake.  Showing Errors and Warnings
