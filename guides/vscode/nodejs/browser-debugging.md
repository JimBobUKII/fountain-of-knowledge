...
* Clicking a link in the [JavaScript debug terminal](nodejs-debugging.md#javascript-debug-terminal).
* Use a [launch config](#launch-configuration) to launch a browser with your app.

We also have more detailed walkthroughs to get started with [React](reactjs-tutorial), [Angular](angular-tutorial), [Vue](vuejs-tutorial), and [Ember](emberjs-tutorial), as well as other debugging [recipes](debugging-recipes).
...
In this section, we'll go into more detail about configurations and features for more advanced debugging scenarios. Instructions for Node.js debugging with [source maps](nodejs-debugging.md#source-maps) and [stepping over external code](nodejs-debugging.md#skipping-uninteresting-code) also apply to browser-based debugging.
...
* `outFiles` - An array of glob patterns for locating generated JavaScript files. See the section on [Source maps](nodejs-debugging.md#source-maps).
* `smartStep`- Try to automatically step over source code that doesn't map to source files. See the section on [Smart stepping](nodejs-debugging.md#smart-stepping).
* `skipFiles` - Automatically skip files covered by these glob patterns. See the section on [Skipping uninteresting code](nodejs-debugging.md#skipping-uninteresting-code).
...
* `port` - Debug port to use. See the section on [Attaching to Node.js](nodejs-debugging.md#attaching-to-browsers).
* `address` - TCP/IP address of the debug port. See the section on [Attaching to Browsers](nodejs-debugging.md#attaching-to-browsers).

## Next steps

* [Debugging](../editor/debugging.md) - Read about general VS Code debugging features.
* [Debugging Recipes](debugging-recipes.md) - Set up debugging for your favorite platform.
